class Api::V1::SiteLevelTaxonomiesController < Api::V1::ApiController
  swagger_controller :site_level_taxonomies, "SiteLevel Taxonomy Terms"

  swagger_api :index do
    summary "Fetches all site level taxonomies in the system"
  ...
  def index
    ...
    begin
      if params[:query]
        @results = SiteLevelTaxonomy.search(params[:query]).records
      else
        @results = SiteLevelTaxonomy.all
      end
  ...
  def show
    @result = SiteLevelTaxonomy.find_by(tid: params[:id])
    render "show"
  end

  #field_site_level_topic_taxonomy
end