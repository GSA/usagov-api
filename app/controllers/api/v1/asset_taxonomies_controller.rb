class Api::V1::AssetTaxonomiesController < Api::V1::ApiController
  swagger_controller :asset_taxonomies, "Asset Taxonomy Terms"
  ...
  def index
    ...
    begin
      if params[:query]
        @results = AssetTaxonomy.search(params[:query]).records
      else
        @results = AssetTaxonomy.all
      end
  ...
  def show
    @result = AssetTaxonomy.find_by(tid: params[:id])
    render "show"
  end
